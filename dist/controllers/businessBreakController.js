...
export const create = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to create business break." });
    }
};
export const getAll = async (req, res) => {
    const date = validateBusinessBreakDate(req.query.date);
    if (!date.isValid)
        return sendBadRequest(res, date.message);
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to retrieve business breaks." });
    }
};
export const getById = async (req, res) => {
    ...
    try {
        ...
        if (!businessBreak) {
            res.status(404).json({ success: false, message: "Business break not found." });
            return;
        }
        res.status(200).json({
        ...
        res.status(500).json({ success: false, message: "Failed to retrieve business break." });
    }
};
export const update = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to update business break." });
    }
};
export const remove = async (req, res) => {
    ...
    if (!validation.isValid)
        return sendBadRequest(res, validation.message);
    try {
        if (!(await service.deleteBusinessBreak(validation.data))) {
            res.status(404).json({ success: false, message: "Business break not found." });
            return;
        }
        res.status(200).json({ success: true, message: "Business break deleted successfully." });
    }
    catch {
        res.status(500).json({ success: false, message: "Failed to delete business break." });
    }
};
//# sourceMappingURL=businessBreakController.js.map