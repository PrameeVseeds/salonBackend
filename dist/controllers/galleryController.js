...
const uploadedPath = (req) => req.file ? path.resolve("uploads", "gallery", req.file.filename) : null;
const removeFile = async (filePath) => { if (filePath)
...
export const create = async (req, res) => {
    ...
    try {
        const image = await service.createGalleryImage({ ...validation.data, image_url: `/uploads/gallery/${req.file.filename}` });
        res.status(201).json({
...
export const getById = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to retrieve gallery image." });
    }
};
...
export const updateStatus = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to update gallery status." });
    }
};
export const updateImage = async (req, res) => {
    ...
    try {
        ...
        await removeFile(filePath);
        res.status(500).json({ success: false, message: "Failed to update gallery image file." });
    }
};
