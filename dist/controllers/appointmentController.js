...
const customerId = (req) => req.customer.id;
...
export const todayBoard = async (_req, res) => {
    try {
...
const changeStatus = async (req, res, action) => {
    const validation = validateAppointmentId(req.params.id);
    if (!validation.isValid)
        return sendBadRequest(res, validation.message);
    try {
...
export const start = (req, res) => changeStatus(req, res, "start");
export const complete = (req, res) => changeStatus(req, res, "complete");
...
export const cancelOwned = async (req, res) => {
    const validation = validateAppointmentId(req.params.id);
    if (!validation.isValid)
        return sendBadRequest(res, validation.message);
