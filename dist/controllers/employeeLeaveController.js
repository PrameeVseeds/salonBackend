...
export const getLeave = async (req, res) => {
    ...
    if (!validation.isValid)
        return sendBadRequest(res, "Invalid employee leave ID.");
    try {
        const leave = await leaveService.getLeave(validation.data);
        if (!leave) {
            res.status(404).json({ success: false, message: "Employee leave not found." });
            return;
        }
...
export const deleteLeave = async (req, res) => {
    ...
    if (!validation.isValid)
        return sendBadRequest(res, "Invalid employee leave ID.");
    try {
        if (!(await leaveService.deleteLeave(validation.data))) {
            res.status(404).json({ success: false, message: "Employee leave not found." });
            return;
        }
...
export const getEmployeeLeaves = async (req, res) => {
    ...
    if (!employee.isValid)
        return sendBadRequest(res, employee.message);
    if (!date.isValid)
...
//# sourceMappingURL=employeeLeaveController.js.map