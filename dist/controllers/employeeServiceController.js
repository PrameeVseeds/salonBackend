...
export const assignService = async (req, res) => {
    ...
    try {
        await assignmentService.assignServiceToEmployee(validation.data.employeeId, validation.data.serviceId);
        res.status(201).json({
            success: true,
...
export const removeService = async (req, res) => {
    ...
    try {
        const removed = await assignmentService.removeServiceFromEmployee(validation.data.employeeId, validation.data.serviceId);
        if (removed === null) {
            res.status(404).json({ success: false, message: "Employee not found." });
            return;
        }
...
//# sourceMappingURL=employeeServiceController.js.map