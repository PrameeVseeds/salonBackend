...
const removeManagedMedia = async (mediaUrl, folderName) => {
    if (mediaUrl?.startsWith(`/uploads/${folderName}/`))
        await removeUploadedFile(path.resolve("uploads", folderName, path.basename(mediaUrl)));
};
...
export const putSettings = async (req, res) => {
    ...
    try {
        const updatedSettings = await settingsService.updateSettings(validation.data);
        res.json({
            success: true,
...
export const patchLogo = async (req, res) => {
    ...
    try {
        ...
        const updatedSettings = await settingsService.updateLogo(`/uploads/settings/${req.file.filename}`);
