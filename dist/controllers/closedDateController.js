...
export const getAll = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Failed to retrieve closed dates." });
    }
};
export const getById = async (req, res) => {
    ...
    try {
        ...
        if (!closedDate) {
            res.status(404).json({ success: false, message: "Closed date not found." });
            return;
        }
        res.status(200).json({
        ...
        res.status(500).json({ success: false, message: "Failed to retrieve closed date." });
    }
};
...
export const remove = async (req, res) => {
    ...
    if (!validation.isValid)
        return sendBadRequest(res, validation.message);
    try {
        if (!(await service.deleteClosedDate(validation.data))) {
            res.status(404).json({ success: false, message: "Closed date not found." });
            return;
        }
        ...
        res.status(500).json({ success: false, message: "Failed to delete closed date." });
    }
};
//# sourceMappingURL=closedDateController.js.map