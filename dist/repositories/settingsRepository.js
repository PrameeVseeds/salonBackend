...
export const getSettings = async () => {
    const [rows] = await pool.execute(`SELECT ${settingsSelectFields} 
    FROM settings 
    WHERE id = 1 
    LIMIT 1`);
    return rows[0] ?? null;
...
export const updateSettings = async (input) => {
    await pool.execute(`INSERT INTO settings (${insertSettingsFields})
        VALUES (1,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ON DUPLICATE KEY 
        UPDATE salon_name=VALUES(salon_name), 
        phone=VALUES(phone), email=VALUES(email), 
        address=VALUES(address), map_url=VALUES(map_url), facebook_url=VALUES(facebook_url), 
