...
const appointmentStartTime = (date, time) => new Date(`${date}T${time}`).getTime();
const isPastOrCurrentTime = (date, time) => appointmentStartTime(date, time) <= Date.now();
const toDateKey = (value) => {
    if (!(value instanceof Date))
        return String(value).slice(0, 10);
    return `${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, "0")}-${String(value.getDate()).padStart(2, "0")}`;
};
const overlaps = (start, end, range) => start < toMinutes(range.end_time) && end > toMinutes(range.start_time);
const MAX_CONCURRENT_CUSTOMER_APPOINTMENTS = 4;
const enforceCustomerAppointmentLimit = async (customerId, date, start, end, excludeAppointmentId, db) => {
    const ranges = await repository.findCustomerAppointmentTimeRanges(customerId, date, excludeAppointmentId, db);
    ...
    const limitReached = checkpoints.some((point) => relevantRanges.filter((range) => toMinutes(range.start_time) <= point && toMinutes(range.end_time) > point).length >= MAX_CONCURRENT_CUSTOMER_APPOINTMENTS);
    if (limitReached)
...
const getScheduleContext = async (query, excludeAppointmentId, db) => {
    const selectedService = await repository.findActiveService(query.serviceId, db);
    if (!selectedService)
        throw new Error("Service not found or inactive.");
    if (query.employeeId !== null &&
        !(await repository.employeeOffersService(query.employeeId, query.serviceId, db))) {
...
const planServiceSegments = async (query, start, excludeAppointmentId, db) => {
    ...
    if (subServices.some((item, index) => query.subServiceIds[index] !== null &&
        (!item || item.service_id !== query.serviceIds[index])))
        return null;
    const segments = [];
...
export const getAvailability = async (query) => {
    ...
    const employeeContexts = assignedEmployeeIds.length
        ? await Promise.all(assignedEmployeeIds.map((employeeId) => getScheduleContext({ ...query, employeeId })))
        : [];
    for (let start = opening; start + duration + buffer <= closing; start += slotStep) {
        if (isPastOrCurrentTime(query.date, toTime(start)))
            continue;
        const serviceBookings = context.serviceAppointments.filter((range) => overlaps(start, start + duration + buffer, range)).length;
        const salonIsAvailable = !context.blocked.some((range) => overlaps(start, start + duration + buffer, range));
        const availableEmployees = employeeContexts.filter((employeeContext) => !employeeContext.unavailable &&
            !employeeContext.blocked.some((range) => overlaps(start, start + duration + buffer, range))).length;
...
export const getAvailableSlots = async (query) => (await getAvailability(query)).slots;
const saveAppointment = async (customerId, input, appointmentId) => {
    const id = await repository.withTransaction(async (connection) => {
        ...
        if (!(await repository.lockService(connection, input.serviceId)))
            throw new Error("Service not found or inactive.");
        const candidates = input.employeeId ? [input.employeeId] : [null];
        ...
        if (input.serviceIds.length > 1 || input.subServiceIds.some(Boolean)) {
            for (const serviceId of input.serviceIds.slice(1))
                if (!(await repository.lockService(connection, serviceId)))
                    throw new Error("Service not found or inactive.");
            ...
            if (appointmentId) {
                ...
                savedId = await repository.insert(connection, { customerId, ...values });
        ...
        for (const candidateId of candidates) {
            if (candidateId !== null && !(await repository.lockEmployee(connection, candidateId)))
                continue;
...
export const createAppointment = async (customerId, input) => {
    const appointment = await saveAppointment(customerId, input);
    await createAppointmentConfirmation(appointment).catch((error) => console.error("Failed to create appointment notification:", error));
    return appointment;
};
export const updateAppointment = (id, customerId, input) => saveAppointment(customerId, input, id);
export const getMyAppointments = (customerId) => repository.findByCustomer(customerId).then(repository.attachAppointmentServices);
export const getOwnedAppointment = (id, customerId) => repository.findOwnedById(id, customerId);
export const deleteOwnedAppointment = (id, customerId) => repository.removeOwned(id, customerId);
export const getAllAppointments = (filters) => repository.findAll(filters).then(repository.attachAppointmentServices);
export const getTodayAppointmentBoard = () => repository.findTodayScheduled();
export const assignEmployeeToAppointment = async (id, employeeId, serviceId) => {
    const appointment = await repository.findById(id);
    if (!appointment)
        throw new Error("Appointment not found.");
...
export const getAvailableEmployeeIdsForAppointment = async (id, serviceId) => {
    const appointment = await repository.findById(id);
    if (!appointment)
        throw new Error("Appointment not found.");
    ...
    return availability.filter((employeeId) => employeeId !== null);
};
export const startAppointment = async (id) => {
    ...
    if (!appointment)
        throw new Error("Appointment not found.");
    await createAppointmentStarted(appointment).catch((error) => console.error("Failed to create appointment start notification:", error));
    return appointment;
};
export const completeAppointment = async (id) => {
    ...
    if (!appointment)
        throw new Error("Appointment not found.");
    await createAppointmentCompletion(appointment).catch((error) => console.error("Failed to create appointment completion notification:", error));
    return appointment;
};
export const processTimedAppointmentStatuses = async () => {
    const dueReminders = await repository.findDueReminders();
    for (const appointment of dueReminders) {
        await createAppointmentReminder(appointment).catch((error) => console.error(`Failed to send reminder for appointment ${appointment.id}:`, error));
    }
    ...
    for (const appointment of overdue) {
        if (await repository.cancelScheduledAsOverdue(appointment.id)) {
            ...
            if (cancelled)
                await createAppointmentCancellation(cancelled).catch((error) => console.error(`Failed to notify cancellation for appointment ${appointment.id}:`, error));
        }
    }
    ...
    for (const appointment of dueForCompletion) {
        if (await repository.updateStatus(appointment.id, "In Progress", "Completed")) {
            ...
            if (completed)
                await createAppointmentCompletion(completed).catch((error) => console.error(`Failed to notify completion for appointment ${appointment.id}:`, error));
        }
    }
};
...
export const cancelCustomerAppointment = async (id, customerId, reason) => {
