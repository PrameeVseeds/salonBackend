...
export const validateAppointmentId = (value) => {
...
export const validateAppointmentRequest = (body) => {
    ...
    const serviceIds = rawServiceIds.map(positiveId).filter((id) => id !== null);
...
export const validateAvailabilityQuery = (query) => {
    ...
    const serviceIds = String(query.serviceIds ?? query.serviceId ?? "").split(",").map(positiveId).filter((id) => id !== null);
    const serviceId = serviceIds[0] ?? null;
...
export const validateAppointmentFilters = (query) => {
