...
export const validateBusinessBreakId = (value: string | string[] | undefined): ValidationResult<number> => {
    if (Array.isArray(value)) return { isValid: false, message: "Invalid business-break ID." };
    const id = Number(value);
    if (!Number.isInteger(id) || id <= 0) return { isValid: false, message: "Invalid business-break ID." };
    return { 
        isValid: true, 
        data: id 
    };
...
export const validateBusinessBreakDate = (value: unknown): ValidationResult<string | undefined> => {
    if (value === undefined) return { isValid: true, data: undefined };
    const date = getString(value);
    if (!date || !isValidDate(date)) return { isValid: false, message: "Date must use valid YYYY-MM-DD format." };
    return { 
        isValid: true, 
        data: date 
    };
...
export const validateBusinessBreak = (body: Record<string, unknown>): ValidationResult<BusinessBreakInput> => {
