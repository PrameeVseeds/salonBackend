...
const getInteger = (value: unknown, mustBePositive: boolean) =>
  typeof value === "number" &&
...
export const validateSettings = (body: Record<string, unknown>,): ValidationResult<SettingsInput> => {
  const salonName = getString(body.salonName ?? body.salon_name);
  const phone = getString(body.phone);
  const email = getString(body.email);
  const address = getString(body.address);
  
  const allowCustomerChooseEmployee = getBoolean(
    body.allowCustomerChooseEmployee ?? body.allow_customer_choose_employee,
  );
  const enableOnlinePayment = getBoolean(
    body.enableOnlinePayment ?? body.enable_online_payment,
  );
  const bookingIntervalMinutes = getInteger(
    body.bookingIntervalMinutes ?? body.booking_interval_minutes,
    true,
  );
  const appointmentBufferMinutes = getInteger(
  ...
  const appointmentReminderMinutes = getInteger(
