...
export const validateRegisterService = (body: Record<string, unknown>,): ValidationResult<RegisterServiceInput> => validateServicePayload(body);

export const validateUpdateService = (body: Record<string, unknown>,): ValidationResult<UpdateServiceInput> => validateServicePayload(body);

export const validateServiceId = (id: string | string[] | undefined,): ValidationResult<number> => {
    if (Array.isArray(id)) {
        return {
            isValid: false,
...
export const validateServiceStatus = (body: Record<string, unknown>,): ValidationResult<boolean> => {
    const isActive = body.is_active ?? body.isActive;
