...
export const getAll = async (_req: Request, res: Response): Promise<void> => {
    ...
    } catch { res.status(500).json({ success: false, message: "Failed to retrieve working hours." }); }
};
...
export const getById = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!hours) { res.status(404).json({ success: false, message: "Working hours not found." }); return; }
...
export const updateStatus = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    ...
    if (!id.isValid) return sendBadRequest(res, id.message);
    if (!status.isValid) return sendBadRequest(res, status.message);
    try {
    ...
    catch { res.status(500).json({ success: false, message: "Failed to update working-hours status." }); }
};
