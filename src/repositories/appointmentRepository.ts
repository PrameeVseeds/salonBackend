...
const fields = "id, customer_id, employee_id, service_id, appointment_date, start_time, end_time, total_amount, notes, created_at, updated_at";
...
export const findByCustomer = async (customerId: number): Promise<AppointmentRow[]> => {
    ...
    (
        ...
        FROM appointments 
        WHERE customer_id = ? 
        ORDER BY appointment_date DESC, start_time DESC`, [customerId]);
    return rows;
};
...
export const findAll = async (filters: AppointmentFilters): Promise<AppointmentRow[]> => {
    ...
    if (filters.date) 
        { conditions.push("appointment_date = ?"); values.push(filters.date); }

    if (filters.employeeId) 
        { conditions.push("employee_id = ?"); values.push(filters.employeeId); }

    if (filters.customerId) 
        { conditions.push("customer_id = ?"); values.push(filters.customerId); }
...
export const lockOwnedAppointment = async (connection: PoolConnection,id: number,customerId: number): Promise<AppointmentRow | null> => {
...
export const insert = async (connection: PoolConnection, values: {
    customerId: number; employeeId: number; serviceId: number; date: string;
    startTime: string; endTime: string; amount: number; notes: string | null;
}): Promise<number> => {
    ...
    (
        ...
        (customer_id, employee_id, service_id, appointment_date, start_time, end_time, total_amount, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [values.customerId, values.employeeId, values.serviceId, values.date, values.startTime, values.endTime, values.amount, values.notes],
    );
    return result.insertId;
};

export const updateOwned = async (connection: PoolConnection, id: number, customerId: number, values: {
    employeeId: number; serviceId: number; date: string; startTime: string; endTime: string; amount: number; notes: string | null;
}): Promise<boolean> => {
    const [result] = await connection.execute<ResultSetHeader>
    (
        `UPDATE appointments 
        SET employee_id = ?, service_id = ?, appointment_date = ?, start_time = ?, end_time = ?, total_amount = ?, notes = ?
         WHERE id = ? AND customer_id = ?`,
        [values.employeeId, values.serviceId, values.date, values.startTime, values.endTime, values.amount, values.notes, id, customerId],
    );
...
export const removeOwned = async (id: number, customerId: number): Promise<boolean> => {
    ...
    ("DELETE FROM appointments WHERE id = ? AND customer_id = ?", [id, customerId]);
    return result.affectedRows > 0;
};
